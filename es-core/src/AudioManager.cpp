...
{
  if (RHGamestationConf::getInstance()->get("audio.bgmusic") == "1")
  {
...
{
  this->init();
  if (currentMusic != NULL && RHGamestationConf::getInstance()->get("audio.bgmusic") == "1")
  {
...
void AudioManager::musicEnd()
{
  ...
  if (runningFromPlaylist && RHGamestationConf::getInstance()->get("audio.bgmusic") == "1")
  {
    playRandomMusic();
  }
}
...
void AudioManager::playCheckSound()
{
  ...
  {
    loadingMusic = "/rhgamestation/share_init/system/.emulationstation/themes/rhgamestation/fx/loading.ogg";
  }
