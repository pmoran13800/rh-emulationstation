...
{
	mRooms.clear();
	std::string lobby = RHGamestationConf::getInstance()->get("global.netplay.lobby");
	auto json_req = RHGamestationSystem::getInstance()->execute("curl -s --connect-timeout 3 -m 3 " + lobby);
...
void GuiNetPlay::populateGridMeta(int i)
{
    ...
    if (std::regex_search(username, std::regex("@RHGAMESTATION")))
    {
        username = std::regex_replace(username, std::regex("@RHGAMESTATION"), " \uF200");
    }
    else if (std::regex_search(frontend, std::regex("@RHGAMESTATION")))
    {
        username = username + " \uF200";
		frontend = std::regex_replace(frontend, std::regex("@RHGAMESTATION"), "");
    }
...
FileData* GuiNetPlay::findGame(std::string gameNameOrHash)
{
	for (auto tmp : SystemData::sSystemVector) {
		if (RHGamestationConf::getInstance()->isInList("global.netplay.systems", tmp->getName())) {
...
std::pair<std::string, std::string> GuiNetPlay::getCoreInfo(const std::string &name) {
    ...
    std::string line;
    std::string filePath = "/rhgamestation/share/system/configs/retroarch.corenames";
...
{
    std::pair<std::string, int> ping = RHGamestationSystem::getInstance()->execute("ping -c 1 -w 1 " + ip + " | grep \"min/avg/max\" | cut -d '/' -f 5");
    if (ping.first != "") {
