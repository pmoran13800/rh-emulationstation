...
GuiHashStart::GuiHashStart(Window* window) : GuiComponent(window), mMenu(window, _("HASH NOW").c_str()), mBusyAnim(window)
{
    ...
    {
        if(RHGamestationConf::getInstance()->isInList("global.netplay.systems", (*it)->getName()))
...
void GuiHashStart::start()
{
    for(auto system : mSystems->getSelectedObjects()) {
    	...
    	std::string command = "/rhgamestation/scripts/rhgamestation-hash.sh -s \"" + system->getName() + "\" -t";

	    auto cmdResult = RHGamestationSystem::getInstance()->execute(command);
        ...
        for(pugi::xml_node fileNode = root.child(tag); fileNode; fileNode = fileNode.next_sibling(tag))
        {
            ...
            std::string cmd = "/rhgamestation/scripts/rhgamestation-hash.sh -f \"" + path.string() + "\"";

	        auto hashResult = RHGamestationSystem::getInstance()->execute(cmd);
