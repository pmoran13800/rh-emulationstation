...
GuiMenu::GuiMenu(Window* window) : GuiComponent(window), mMenu(window, _("MAIN MENU").c_str()), mVersion(window) {
  ...
  bool bartop =  RHGamestationConf::getInstance()->get("emulationstation.menu") == "bartop";

  //KODI
    if (RHGamestationConf::getInstance()->get("kodi.enabled") == "1") {
        addEntryWithHelp(_("KODI MEDIA CENTER").c_str(), _(MenuMessages::START_KODI_HELP_MSG), mMenuTheme->menuText.color, true,
                 [this] {
                     Window* window = mWindow;
                     if (!RHGamestationSystem::getInstance()->launchKodi(window)) {
...
void GuiMenu::createInputTextRow(GuiSettings *gui, std::string title, const char *settingsID, bool password, std::string help = "") {
  ...
  ed = std::make_shared<TextComponent>(window, ((password && RHGamestationConf::getInstance()->get(settingsID) != "") ?
      "*********" : RHGamestationConf::getInstance()->get(settingsID)),
  ...
  auto updateVal = [ed, settingsID, password](const std::string &newVal) {
    ...
    RHGamestationConf::getInstance()->set(settingsID, newVal);
  }; // ok callback (apply new value to ed)

  row.makeAcceptInputHandler([this, title, updateVal, settingsID] {
    if (Settings::getInstance()->getBool("UseOSK")) {
      mWindow->pushGui(
          new GuiTextEditPopupKeyboard(mWindow, title, RHGamestationConf::getInstance()->get(settingsID),
                         updateVal, false));
    } else {
      mWindow->pushGui(
          new GuiTextEditPopup(mWindow, title, RHGamestationConf::getInstance()->get(settingsID),
                     updateVal, false));
    }
  });
...
void GuiMenu::menuSystem(){
  ...
  std::vector<std::string> availableStorage = RHGamestationSystem::getInstance()->getAvailableStorageDevices();
  std::string selectedStorage = RHGamestationSystem::getInstance()->getCurrentStorage();
  ...
  std::string language = RHGamestationConf::getInstance()->get("system.language");
  ...
  s->addSaveFunc([window, language_choice, language, optionsStorage, selectedStorage] {
    ...
    if (optionsStorage->changed()) {
      RHGamestationSystem::getInstance()->setStorage(optionsStorage->getSelected());
      reboot = true;
    }

    if (language != language_choice->getSelected()) {
      RHGamestationConf::getInstance()->set("system.language", language_choice->getSelected());
...
void GuiMenu::menuUpdates(){
  ...
  updates_enabled->setState(RHGamestationConf::getInstance()->get("updates.enabled") == "1");
  ...
  if (RHGamestationUpgrade::getInstance()->canUpdate()) {
    ...
    updateGui->addSubMenu(_("UPDATE CHANGELOG"), [this] {
      ...
      if (changelog != "") {
        ...
        mWindow->displayScrollMessage(_("AN UPDATE IS AVAILABLE FOR YOUR RH GAMESTATION"),
            _("UPDATE VERSION:") + " " + updateVersion + "\n" +
            _("UPDATE CHANGELOG:") + "\n" + message);
      } else {
        mWindow->displayMessage(_("AN UPDATE IS AVAILABLE FOR YOUR RH GAMESTATION"));
      }
  ...
  auto updatesTypeComp = std::make_shared<OptionListComponent<std::string> >(mWindow,
      _("UPDATE TYPE"), false);
  std::string updatesType = RHGamestationConf::getInstance()->get("updates.type");
  // Stable or nothing
  ...
  updateGui->addSaveFunc([updates_enabled, updatesTypeComp] {
    RHGamestationConf::getInstance()->set("updates.enabled", updates_enabled->getState() ? "1" : "0");
    if(updatesTypeComp->getSelected() == "stable"){
      RHGamestationConf::getInstance()->set("updates.type", updatesTypeComp->getSelected());
    }
    RHGamestationConf::getInstance()->saveRHGamestationConf();
  });
...
void GuiMenu::menuGameSettings(){
  ...
  if (RHGamestationConf::getInstance()->get("emulationstation.menu") != "bartop") {
    // Screen ratio choice
    ...
    s->addSaveFunc([ratio_choice] {
      RHGamestationConf::getInstance()->set("global.ratio", ratio_choice->getSelected());
      RHGamestationConf::getInstance()->saveRHGamestationConf();
    });
  }
  // smoothing
  auto smoothing_enabled = std::make_shared<SwitchComponent>(mWindow);
  smoothing_enabled->setState(RHGamestationConf::getInstance()->get("global.smooth") == "1");
  ...
  rewind_enabled->setState(RHGamestationConf::getInstance()->get("global.rewind") == "1");
  ...
  autosave_enabled->setState(RHGamestationConf::getInstance()->get("global.autosave") == "1");
  ...
  auto shaders_choices = std::make_shared<OptionListComponent<std::string> >(mWindow,
      _("SHADERS SET"), false);
  std::string currentShader = RHGamestationConf::getInstance()->get("global.shaderset");
  ...
  integerscale_enabled->setState(RHGamestationConf::getInstance()->get("global.integerscale") == "1");
  ...
  s->addSaveFunc([integerscale_enabled] {
    RHGamestationConf::getInstance()->set("global.integerscale", integerscale_enabled->getState() ? "1" : "0");
    RHGamestationConf::getInstance()->saveRHGamestationConf();
  });
    ...
    // Retroachievements
    {
      std::function<void()> openGui = [this] {
        ...
        retroachievements_enabled->setState(
            RHGamestationConf::getInstance()->get("global.retroachievements") == "1");
        ...
        retroachievements_hardcore_enabled->setState(
            RHGamestationConf::getInstance()->get("global.retroachievements.hardcore") ==
            "1");
        ...
        retroachievements->addSaveFunc(
            [retroachievements_enabled, retroachievements_hardcore_enabled] {
              RHGamestationConf::getInstance()->set("global.retroachievements", retroachievements_enabled->getState() ? "1" : "0");
              RHGamestationConf::getInstance()->set("global.retroachievements.hardcore", retroachievements_hardcore_enabled->getState() ? "1" : "0");
              RHGamestationConf::getInstance()->saveRHGamestationConf();
            });
    ...
    if (RHGamestationConf::getInstance()->get("emulationstation.menu") != "bartop") {
      // Netplay
      {
        std::function<void()> openGui = [this] {
          ...
          netplay_enabled->setState(RHGamestationConf::getInstance()->get("global.netplay") == "1");
          ...
          std::string currentMitm = RHGamestationConf::getInstance()->get("global.netplay.relay");
          ...
          netplay->addSaveFunc(
              [netplay_enabled, mitm_choices] {
                ...
                RHGamestationConf::getInstance()->set("global.netplay", netplay_enabled->getState() ? "1" : "0");
                RHGamestationConf::getInstance()->set("global.netplay.relay", mitm);
                RHGamestationConf::getInstance()->saveRHGamestationConf();
              });
  ...
  s->addSaveFunc([smoothing_enabled, rewind_enabled, shaders_choices, autosave_enabled] {
    RHGamestationConf::getInstance()->set("global.smooth", smoothing_enabled->getState() ? "1" : "0");
    RHGamestationConf::getInstance()->set("global.rewind", rewind_enabled->getState() ? "1" : "0");
    RHGamestationConf::getInstance()->set("global.shaderset", shaders_choices->getSelected());
    RHGamestationConf::getInstance()->set("global.autosave", autosave_enabled->getState() ? "1" : "0");
    RHGamestationConf::getInstance()->saveRHGamestationConf();
  });
  mWindow->pushGui(s);
}
...
void GuiMenu::menuControllers() {
          ...
          for (std::vector<std::string>::iterator controllerString = ((std::vector<std::string> *) controllers)->begin();
            ...
            std::function<void()> pairController = [this, window, pairGui, controllerString, deletePairGui] {
              window->pushGui(new GuiLoading(window, [controllerString] {
                bool paired = RHGamestationSystem::getInstance()->pairBluetooth(*controllerString);
  ...
  row.makeAcceptInputHandler([window, this, s, showControllerList] {
    window->pushGui(new GuiLoading(window, [] {
      auto s = RHGamestationSystem::getInstance()->scanBluetooth();
      return (void *) s;
    }, showControllerList));
  });
  ...
  row.makeAcceptInputHandler([window, this, s] {
    RHGamestationSystem::getInstance()->forgetBluetoothControllers();
...
void GuiMenu::menuSoundSettings(){
  ...
  sounds_enabled->setState(!(RHGamestationConf::getInstance()->get("audio.bgmusic") == "0"));
  ...
  std::string currentDevice = RHGamestationConf::getInstance()->get("audio.device");
  if (currentDevice.empty()) currentDevice = "auto";

  std::vector<std::string> availableAudio = RHGamestationSystem::getInstance()->getAvailableAudioOutputDevices();
  std::string selectedAudio = RHGamestationSystem::getInstance()->getCurrentAudioOutputDevice();
  ...
  if (RHGamestationConf::getInstance()->get("emulationstation.menu") != "bartop") {
    ...
    auto setAudioDevice = [](const std::string &newVal) {
      RHGamestationSystem::getInstance()->setAudioOutputDevice(newVal);
    };
  ...
  s->addSaveFunc([optionsAudio, currentDevice, sounds_enabled, volume] {
    RHGamestationConf::getInstance()->set("audio.volume", std::to_string((int) round(volume->getValue())));

    RHGamestationConf::getInstance()->set("audio.bgmusic", sounds_enabled->getState() ? "1" : "0");
    if (!sounds_enabled->getState())
      AudioManager::getInstance()->stopMusic();
    if (currentDevice != optionsAudio->getSelected()) {
      RHGamestationConf::getInstance()->set("audio.device", optionsAudio->getSelected());
    }
    RHGamestationConf::getInstance()->saveRHGamestationConf();
  });
...
void GuiMenu::menuNetworkSettings(){
  ...
  auto status = std::make_shared<TextComponent>(mWindow, RHGamestationSystem::getInstance()->ping() ? _( "CONNECTED")
  ...
  auto ip = std::make_shared<TextComponent>(mWindow, RHGamestationSystem::getInstance()->getIpAdress(),
      mMenuTheme->menuText.font, mMenuTheme->menuText.color);
  ...
  bool baseEnabled = RHGamestationConf::getInstance()->get("wifi.enabled") == "1";
  enable_wifi->setState(baseEnabled);
  s->addWithLabel(enable_wifi, _("ENABLE WIFI"), _(MenuMessages::NETWORK_WIFI_HELP_MSG));

  //SSID
  std::string baseSSID = RHGamestationConf::getInstance()->get("wifi.ssid");
  auto WifiSSID = [this, baseSSID, enable_wifi, baseEnabled] (GuiSettings *gui, std::string title,
    ...
    row.makeAcceptInputHandler([this, updateVal, enable_wifi, baseEnabled] {
      ...
      auto updateValue = [this, updateVal, SSID](const std::string &newVal) {
        RHGamestationConf::getInstance()->set("wifi.ssid", newVal);
        updateVal(newVal);
        delete SSID;
      };
      ...
      if (enable_wifi->getState()) {
        std::vector<std::string> availableSSID = RHGamestationSystem::getInstance()->getAvailableWiFiSSID(baseEnabled);
        RHGamestationConf::getInstance()->set("wifi.enabled", "1");
  ...
  const std::string baseKEY = RHGamestationConf::getInstance()->get("wifi.key");
  ...
  s->addSaveFunc([baseEnabled, baseSSID, baseKEY, enable_wifi, window] {
    ...
    std::string newSSID = RHGamestationConf::getInstance()->get("wifi.ssid");
    std::string newKey = RHGamestationConf::getInstance()->get("wifi.key");

    if (wifienabled) {
      if (baseSSID != newSSID
        || baseKEY != newKey
        || !baseEnabled) {
        if (RHGamestationSystem::getInstance()->enableWifi(newSSID, newKey)) {
    ...
    else if (baseEnabled || RHGamestationConf::getInstance()->get("wifi.enabled") == "1"){
      RHGamestationSystem::getInstance()->disableWifi();
    }

    RHGamestationConf::getInstance()->set("wifi.enabled", wifienabled ? "1" : "0");
    RHGamestationConf::getInstance()->saveRHGamestationConf();
  });
  mWindow->pushGui(s);
}
...
void GuiMenu::menuAdvancedSettings(){
  ...
  s->addSaveFunc([overclock_choice, overclockWarning, window]  {
    ...
    if (Settings::getInstance()->getString("Overclock") != overclock_choice->getSelected()) {
      ...
      RHGamestationSystem::getInstance()->setOverclock(overclock_choice->getSelected());
      reboot = true;
    }
    RHGamestationConf::getInstance()->saveRHGamestationConf();
    if (reboot) {
        ...
        window->pushGui(
            new GuiMsgBox(window, _("TURBO AND EXTREM OVERCLOCK PRESETS MAY CAUSE SYSTEM UNSTABILITIES, SO USE THEM AT YOUR OWN RISK.\nIF YOU CONTINUE, THE SYSTEM WILL REBOOT NOW."),
                _("YES"), [window] {
                  ...
                  RHGamestationSystem::getInstance()->setOverclock("none");
                  Settings::getInstance()->saveFile();
                }));
      } else {
        window->pushGui(
  ...
  // BOOT
  {
    std::function<void()> openGui = [this] {
      ...
      auto kodiAtStart = std::make_shared<SwitchComponent>(mWindow, RHGamestationConf::getInstance()->get("kodi.atstartup") == "1");
      bootGui->addWithLabel(kodiAtStart, _("KODI AT START"), _(MenuMessages::ADVANCED_KODI_AT_START_HELP_MSG));
      // Gamelists only
      bool gamelistOnly = RHGamestationConf::getInstance()->get("emulationstation.gamelistonly") == "1";
      ...
      // Selected System
      std::string selectedsystem = RHGamestationConf::getInstance()->get(
      ...
      // Boot on gamelist
      bool bootOnGamelist =
          RHGamestationConf::getInstance()->get("emulationstation.bootongamelist") == "1";
      ...
      // Hide system view
      bool hidesystemview =
          RHGamestationConf::getInstance()->get("emulationstation.hidesystemview") == "1";
      ...
      bool basicgamelistview =
          RHGamestationConf::getInstance()->get("emulationstation.forcebasicgamelistview") == "1";
      ...
      bootGui->addSaveFunc(
          [gamelistOnlyComp, system_choices, kodiAtStart, bootOnGamelistComp, hidesystemviewComp, basicgamelistviewComp] {
            RHGamestationConf::getInstance()->set("kodi.atstartup", kodiAtStart->getState() ? "1" : "0");
            RHGamestationConf::getInstance()->set("emulationstation.gamelistonly", gamelistOnlyComp->getState() ? "1" : "0");
            RHGamestationConf::getInstance()->set("emulationstation.selectedsystem", system_choices->getSelected());
            RHGamestationConf::getInstance()->set("emulationstation.bootongamelist", bootOnGamelistComp->getState() ? "1" : "0");
            RHGamestationConf::getInstance()->set("emulationstation.hidesystemview", hidesystemviewComp->getState() ? "1" : "0");
            RHGamestationConf::getInstance()->set("emulationstation.forcebasicgamelistview", basicgamelistviewComp->getState() ? "1" : "0");
            RHGamestationConf::getInstance()->saveRHGamestationConf();
          });
  ...
  //Kodi
  {
    std::function<void()> openGui = [this] {
      ...
      kodiEnabled->setState(RHGamestationConf::getInstance()->get("kodi.enabled") == "1");
      ...
      kodiX->setState(RHGamestationConf::getInstance()->get("kodi.xbutton") == "1");
      ...
      kodiGui->addSaveFunc([kodiEnabled, kodiAtStart, kodiX] {
        RHGamestationConf::getInstance()->set("kodi.enabled", kodiEnabled->getState() ? "1" : "0");
        RHGamestationConf::getInstance()->set("kodi.atstartup", kodiAtStart->getState() ? "1" : "0");
        RHGamestationConf::getInstance()->set("kodi.xbutton", kodiX->getState() ? "1" : "0");
        RHGamestationConf::getInstance()->saveRHGamestationConf();
      });
  ...
  //Security
  {
    std::function<void()> openGui = [this] {
      ...
      securityEnabled->setState(
          RHGamestationConf::getInstance()->get("system.security.enabled") == "1");
      securityGui->addWithLabel(securityEnabled, _("ENFORCE SECURITY"), _(MenuMessages::ADVANCED_ENFORCE_SECURITY_HELP_MSG));

      auto rootpassword = std::make_shared<TextComponent>(mWindow, RHGamestationSystem::getInstance()->getRootPassword(),
          mMenuTheme->menuText.font, mMenuTheme->menuText.color);
      ...
      securityGui->addSaveFunc([this, securityEnabled] {
        ...
        if (securityEnabled->changed()) {
          RHGamestationConf::getInstance()->set("system.security.enabled", securityEnabled->getState() ? "1" : "0");
          RHGamestationConf::getInstance()->saveRHGamestationConf();
          reboot = true;
        }
  ...
  s->addSaveFunc([overscan_enabled] {
    if (Settings::getInstance()->getBool("Overscan") != overscan_enabled->getState()) {
      ...
      RHGamestationSystem::getInstance()->setOverscan(overscan_enabled->getState());
    }
  });
  ...
  auto manager = std::make_shared<SwitchComponent>(mWindow, RHGamestationConf::getInstance()->get("system.manager.enabled") == "1");
  s->addWithLabel(manager, _("RHGAMESTATION MANAGER"), _(MenuMessages::ADVANCED_MANAGER_HELP_MSG));
  s->addSaveFunc([manager] {
    RHGamestationConf::getInstance()->set("system.manager.enabled", manager->getState() ? "1" : "0");
    RHGamestationConf::getInstance()->saveRHGamestationConf();;
  });
  // RHGamestation API
  auto rhgamestationApi = std::make_shared<SwitchComponent>(mWindow, RHGamestationConf::getInstance()->get("system.api.enabled") == "1");
  s->addWithLabel(rhgamestationApi, _("RHGAMESTATION API"), _(MenuMessages::ADVANCED_API_HELP_MSG));
  s->addSaveFunc([rhgamestationApi] {
    RHGamestationConf::getInstance()->set("system.api.enabled", rhgamestationApi->getState() ? "1" : "0");
    RHGamestationConf::getInstance()->saveRHGamestationConf();;
  });
  mWindow->pushGui(s);
}
...
void GuiMenu::menuQuit(){
  ...
  row.makeAcceptInputHandler([window] {
    window->pushGui(new GuiMsgBox(window, _("REALLY RESTART?"), _("YES"), [] {
        if (RHGamestationSystem::getInstance()->reboot() != 0) {
  ...
  row.makeAcceptInputHandler([window] {
    window->pushGui(new GuiMsgBox(window, _("REALLY SHUTDOWN?"), _("YES"), [] {
        if (RHGamestationSystem::getInstance()->shutdown() != 0) {
          LOG(LogWarning) << "Shutdown terminated with non-zero result!";
          }
        }, _("NO"), nullptr));
  });
  ...
  row.makeAcceptInputHandler([window] {
    window->pushGui(new GuiMsgBox(window, _("REALLY SHUTDOWN WITHOUT SAVING METADATAS?"), _("YES"), [] {
        if (RHGamestationSystem::getInstance()->fastShutdown() != 0) {
          LOG(LogWarning) << "Shutdown terminated with non-zero result!";
          }
        }, _("NO"), nullptr));
  });
...
void GuiMenu::popSystemConfigurationGui(SystemData *systemData) const {
    ...
    EmulatorDefaults emulatorDefaults = RHGamestationSystem::getInstance()->getEmulatorDefaults(systemData->getName());
    ...
    std::string currentEmulator = RHGamestationConf::getInstance()->get(systemData->getName() + ".emulator");
    ...
    emu_choice->setSelectedChangedCallback([this, systemData, emulatorDefaults, core_choice](std::string emulatorName) {
        ...
        std::string currentCore = RHGamestationConf::getInstance()->get(systemData->getName() + ".core");
    ...
    // smoothing
    auto smoothing_enabled = std::make_shared<SwitchComponent>(mWindow);
    smoothing_enabled->setState(RHGamestationConf::getInstance()->get(systemData->getName() + ".smooth", RHGamestationConf::getInstance()->get("global.smooth")) == "1");
    systemConfiguration->addWithLabel(smoothing_enabled, _("SMOOTH GAMES"), _(MenuMessages::GAME_SMOOTH_HELP_MSG));
    // rewind
    auto rewind_enabled = std::make_shared<SwitchComponent>(mWindow);
    rewind_enabled->setState(RHGamestationConf::getInstance()->get(systemData->getName() + ".rewind", RHGamestationConf::getInstance()->get("global.rewind")) == "1");
    systemConfiguration->addWithLabel(rewind_enabled, _("REWIND"), _(MenuMessages::GAME_REWIND_HELP_MSG));
    // autosave
    auto autosave_enabled = std::make_shared<SwitchComponent>(mWindow);
    autosave_enabled->setState(RHGamestationConf::getInstance()->get(systemData->getName() + ".autosave", RHGamestationConf::getInstance()->get("global.autosave")) == "1");
    ...
    systemConfiguration->addSaveFunc(
            [systemData, smoothing_enabled, rewind_enabled, ratio_choice, emu_choice, core_choice, autosave_enabled] {
                if (ratio_choice->changed()) {
                    RHGamestationConf::getInstance()->set(systemData->getName() + ".ratio", ratio_choice->getSelected());
                }
                if (rewind_enabled->changed()) {
                    RHGamestationConf::getInstance()->set(systemData->getName() + ".rewind", rewind_enabled->getState() ? "1" : "0");
                }
                if (smoothing_enabled->changed()) {
                    RHGamestationConf::getInstance()->set(systemData->getName() + ".smooth", smoothing_enabled->getState() ? "1" : "0");
                }
                // always save both core and emulator
                // this is required to distinguish default
                if (emu_choice->changed() || core_choice->changed()) {
                    RHGamestationConf::getInstance()->set(systemData->getName() + ".emulator", emu_choice->getSelected());
                    RHGamestationConf::getInstance()->set(systemData->getName() + ".core", core_choice->getSelected());
                }
                if (autosave_enabled->changed()) {
                    RHGamestationConf::getInstance()->set(systemData->getName() + ".autosave", autosave_enabled->getState() ? "1" : "0");
                }
                RHGamestationConf::getInstance()->saveRHGamestationConf();
            });
...
std::shared_ptr<OptionListComponent<std::string>> GuiMenu::createRatioOptionList(Window *window, std::string configname) const {
    ...
    std::string currentRatio = RHGamestationConf::getInstance()->get(configname + ".ratio");
