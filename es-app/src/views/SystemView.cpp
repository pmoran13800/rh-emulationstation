...
bool SystemView::input(InputConfig* config, Input input)
{
	if(input.value != 0)
	{
        ...
        if(	config->isMappedTo("x", input) && input.value) {
		    bool kodiEnabled = RHGamestationConf::getInstance()->get("kodi.enabled") == "1";
		    bool kodiX = RHGamestationConf::getInstance()->get("kodi.xbutton") == "1";
		    bool netplay = RHGamestationConf::getInstance()->get("global.netplay") == "1";
		    ...
		    if (kodiEnabled && kodiX && !launchKodi && !mWindow->isShowingPopup()) {
		        if (netplay) {
                    ...
                    row.makeAcceptInputHandler([this, s] {
                        launchKodi = true;
                        if( ! RHGamestationSystem::getInstance()->launchKodi(mWindow)) {
		...
		if(config->isMappedTo("select", input) && RHGamestationConf::getInstance()->get("emulationstation.menu") != "none")
		{
...
std::vector<HelpPrompt> SystemView::getHelpPrompts()
 {
	...
	prompts.push_back(HelpPrompt("b", _("SELECT")));
	 if (RHGamestationConf::getInstance()->get("kodi.enabled") == "1" && RHGamestationConf::getInstance()->get("kodi.xbutton") == "1") {
	 ...
	 } else if (RHGamestationConf::getInstance()->get("global.netplay") == "1") {
