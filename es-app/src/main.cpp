	...
	*errorString = NULL;
	...
	if(SystemData::sSystemVector.size() == 0)
	{
		...
		*errorString = "WE CAN'T FIND ANY SYSTEMS!\n"
		  ...
		  "\n"
		  "VISIT RHGAMESTATION.FR FOR MORE INFORMATION.";
		return false;
	}
...
int main(int argc, char* argv[])
{
	...
	archFile.open("/rhgamestation/rhgamestation.arch");
	...
	RHGamestationConf* rhgamestationConf = RHGamestationConf::getInstance();
	if(rhgamestationConf->get("kodi.enabled") == "1" && rhgamestationConf->get("kodi.atstartup") == "1"){
		RHGamestationSystem::getInstance()->launchKodi(&window);
	}
	RHGamestationSystem::getInstance()->getIpAdress();
	// UPDATED VERSION MESSAGE
    std::string changelog = RHGamestationUpgrade::getInstance()->getChangelog();
    if (changelog != "") {
        ...
        window.pushGui(
                new GuiMsgBoxScroll(
						...
						message, _("OK"),
                        [] {
                            RHGamestationUpgrade::getInstance()->updateLastChangelogFile();
  ...
  NetPlayThread netPlayThread(&window, NetPlayPopupEvent);
	/*if (RHGamestationConf::getInstance()->get("global.netplay") == "1")
	{
		...
		while(SDL_PollEvent(&event))
		{
			switch(event.type)
			{
				...
				case SDL_QUIT:
					running = false;
					break;
				case RHGamestationSystem::SDL_FAST_QUIT | RHGamestationSystem::SDL_RB_REBOOT:
					running = false;
					doReboot = true;
					Settings::getInstance()->setBool("IgnoreGamelist", true);
					break;
				case RHGamestationSystem::SDL_FAST_QUIT | RHGamestationSystem::SDL_RB_SHUTDOWN:
					running = false;
					doShutdown = true;
					Settings::getInstance()->setBool("IgnoreGamelist", true);
					break;
				case SDL_QUIT | RHGamestationSystem::SDL_RB_REBOOT:
					running = false;
					doReboot = true;
					break;
				case SDL_QUIT | RHGamestationSystem::SDL_RB_SHUTDOWN:
					running = false;
					doShutdown = true;
					break;
				default:
